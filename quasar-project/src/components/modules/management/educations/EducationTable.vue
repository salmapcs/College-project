<template>
    <q-table :rows="rows" :columns="columns">
      <template v-slot:top>
        <div class="row items-center">
          <div class="text-h5 q-ma-sm">Education</div>
  ...
  export default {
    ...
    methods: {
      async fetchData () {
        try {
          let httpClient = await this.$api.get('/items/education')
      ...
      async deleteData (id) {
        this.$q.dialog({
          ...
          try {
            let httpClient = await this.$api.delete('/items/education/' + id)
            this.fetchData()
            this.$q.dialog({
    ...
    created () {
      this.$mitt.on('module-data-changed:education', this.fetchData)
      this.fetchData()
    }
  }
  </script>
